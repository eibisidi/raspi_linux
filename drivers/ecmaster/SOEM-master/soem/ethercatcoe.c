...
// end slave bug prevention code
			...
			direction = (context->slavelist[Slave].SM[iSM].SMflags & 0x0C) >> 2;
			
			if (0 == direction  //SII SM方向
				&& tSM != 4)	//通过COE读取的SM类型
			{
				pr_err("Slave:%u SM%u tSm:%u direction:%u\n", Slave, iSM, tSM, direction);
			}
			
			if (1 == direction	//write
				&& tSM != 3)
			{
				pr_err("Slave:%u SM%u tSm:%u direction:%u\n", Slave, iSM, tSM, direction);
			}
            ...
            if (tSM == 0)
            {
               context->slavelist[Slave].SM[iSM].SMflags =
			    ...
			    pr_info("--------------%s slave%u, SM%d Flags:0x%x\n, ", __FUNCTION__, Slave, iSM, context->slavelist[Slave].SM[iSM].SMflags );
            }
            if ((tSM == 3) || (tSM == 4))
            {
               /* read the assign PDO */
...
// end slave bug prevention code
		 ...
		 if (iSM == 3 && tSM == 3)
		{
		 ...
		 direction = (context->slavelist[Slave].SM[iSM].SMflags & 0x0C) >> 2;
		 //pr_err("Slave:%u SM%u tSm:%u SMFlags:0x%x direction:%u\n", Slave, iSM, tSM, context->slavelist[Slave].SM[iSM].SMflags, direction);
		 if (0 == direction  //read
			 && tSM == 3)	 //output SM
		 {
			pr_err("Slave:%u SM%u tSm:%u direction:%u\n", Slave, iSM, tSM, direction);
			//context->slavelist[Slave].SMtype[iSM] |= 0x00000004;		//turn write on
			 context->slavelist[Slave].SM[iSM].SMflags  = 0x10064;
		 }
		 ...
		 {
			 pr_err("Slave:%u SM%u tSm:%u direction:%u\n", Slave, iSM, tSM, direction);
			 //context->slavelist[Slave].SMtype[iSM] &= 0xFFFFFFFB;		//turn read on
			 context->slavelist[Slave].SM[iSM].SMflags = 0x10020;
		 }
