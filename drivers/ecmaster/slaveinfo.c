...
char* SDO2string(uint16 slave, uint16 index, uint8 subidx, uint16 dtype)
{
	...
	else
	{
		...
		case ECT_INTEGER64:
			i64 = (int64*) &usdo[0];
			sprintf(str, "0x%16.16llx / %lld", *i64, *i64);
			break;
		case ECT_UNSIGNED8:
			u8 = (uint8*) &usdo[0];
		...
		case ECT_UNSIGNED64:
			u64 = (uint64*) &usdo[0];
			sprintf(str, "0x%16.16llx / %llu", *u64, *u64);
			break;
		case ECT_REAL32:
			sr = (float*) &usdo[0];
			//sprintf(str, "%f", *sr);
		...
		for (iSM = 2 ; iSM <= nSM ; iSM++)
		{
			...
			if (wkc > 0)
			{
				...
				if (tSM == 3) // outputs
				{
					...
					Tsize = si_PDOassign(slave, ECT_SDO_PDOASSIGN + iSM, (int)(ec_slave[slave].outputs - ec_slave[0].outputs), outputs_bo );
					outputs_bo += Tsize;
				}
				if (tSM == 4) // inputs
				{
					...
					printf("  SM%1d inputs\n     addr b   index: sub bitl data_type    name\n", iSM);
					Tsize = si_PDOassign(slave, ECT_SDO_PDOASSIGN + iSM, (int)(ec_slave[slave].inputs - ec_slave[0].inputs), inputs_bo );
					inputs_bo += Tsize;
				}
			}
		}
