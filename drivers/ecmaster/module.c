...
static int eck_init(eck_t *eck, int index)
{
	...
	ret = eck_cdev_init(&eck->cdev, eck, device_number);
	if (ret)
		goto out_return;
...
static int ecmaster_init_proc(void *data)
{
	...
	if (FALSE == ec_init(IFACE)) {//配置EtherCAT网卡设备
...
static int check_hack_data(void)
{
	...
	if (of_property_read_string(np, "serial-number", &serial)) {//读取/proc/cpuinfo中Serial，16字节 0结尾
		of_node_put(np);
	...
	if (IS_ERR(filep)) {
		pr_err("ec_check_hack_data Open file %s error\n", NET_DRIVER);
		goto out;
	}
	...
	if (retval != sizeof(elf_ex)) {
		pr_err("ec_check_hack_data kernel_read ELF header failed, retval = %d.\n", retval);
		goto out;
	}
	...
	if (retval != j) {
		pr_err("ec_check_hack_data kernel_read Section Headers failed, retval = %d.\n", retval);
	...
	if (retval != elf_shstrtab->sh_size) {
		pr_err("ec_check_hack_data kernel_read .shstrtab  failed, retval = %d.\n", retval);
	...
	for (i = 0; i < elf_ex.e_shnum; i++, ++elf_pshdata) {
		if (0 == strcmp(sh_names + elf_pshdata->sh_name, HACK_SECTION)	//段名称匹配
		    && SHT_PROGBITS == elf_pshdata->sh_type) {
			break;
		}
	}

	if (i >= elf_ex.e_shnum) {
		pr_err("ec_check_hack_data HACK section not found.\n");
		error = -ENOEXEC;
		goto out_free_sh_names;
	}
	...
	if (retval != HACK_DATA_SIZE) {
		pr_err("ec_check_hack_data read HACK sectioin data failed, retval = %d.\n", retval);
		error = -ENOEXEC;
		goto out_free_sh_names;
	}

	retval = memcmp(ORIGINAL_DATA, actual_data, HACK_DATA_SIZE);
	if (0 == retval) {//write new data
...
static int __init ecmaster_init_module(void)
{
	...
	if (check_hack_data() < 0) {//绑定CPU验证失败
		return -EPERM;
	}
#endif
